...
export default function MonthlyReport() {
  ...
  const fetchReportData = async (id: string, month: number, year: number) => {
    ...
    try {
      ...
      if (snapshot) {
        ...
        setIsSaved(true);
        if (typeof snapshot.report_data === 'string') {
          setReportData(JSON.parse(snapshot.report_data));
        } else {
          setReportData(snapshot.report_data); 
        }
        setLoading(false);
        return;
      }
      ...
      // C. Safe String Date Boundaries for Time-Series (Handles both YYYY-MM-DD and YYYY-MM-DDTHH)
      ...
      if (receiptsErr) console.error("Receipts query error: ", receiptsErr);
      ...
      // D. Fetch Daily Logs to sum meals served
      ...
      if (logsErr) console.error("Logs query error: ", logsErr);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        report_data: JSON.stringify(reportData) // JSONB payload lock
      };
      ...
      setIsSaved(true);
      alert("मासिक अहवाल यशस्वीरित्या सेव्ह झाला! (Report Saved Successfully)");
    } catch (error: any) {
  ...
  return (
    <Layout>
      <div className="w-[95%] max-w-[1400px] mx-auto mt-4 pb-20 print:p-0 print:m-0 print:max-w-full print:bg-white">
        ...
        <div className="mb-6 p-5 bg-white rounded-xl shadow-xl border border-slate-200 print:hidden flex flex-col md:flex-row gap-5 justify-end items-center">
          ...
          <div className="flex gap-3">
            {!isSaved ? (
              <button onClick={handleSave} disabled={loading} className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-lg font-black uppercase text-sm flex items-center gap-2 transition-all shadow-lg shadow-indigo-500/30 font-[Inter]">
        ...
        {/* Printable ZP Landscape Layout */}
        <div className="bg-white p-2 text-black print:p-0">
          {/* Inject landscape rule strictly into this document */}
          ...
          {loading ? (
            ...
            <div className="border-[2px] border-black p-4 md:p-6 shadow-sm print:border-none print:shadow-none font-['Inter']">
              ...
              <div className="text-center mb-6 border-b-[2px] border-black pb-4">
                <h1 className="text-xl md:text-2xl font-black mb-1">प्रधानमंत्री पोषण शक्ती निर्माण योजना - मासिक अहवाल</h1>
                <h2 className="text-sm font-bold text-gray-700">शाळेचे नाव: {schoolName || '____________________'} | माहे: {marathiMonths[selectedMonth-1]} {selectedYear}</h2>
              </div>
                ...
                <table className="w-full border-collapse border border-black text-xs md:text-sm whitespace-nowrap mb-6 print:border-[1.5px]">
                  <thead>
                    <tr className="bg-gray-100 print:bg-transparent">
                      ...
                      <th className="border border-black print:border-[1.5px] p-2 md:p-3 text-right text-[10px] md:text-xs">मागील शिल्लक<br/>(Opening Balance)</th>
                      ...
                      <th className="border border-black print:border-[1.5px] p-2 md:p-3 text-right text-[10px] md:text-xs">एकूण शिजवले<br/>(Consumed)</th>
                      <th className="border border-black print:border-[1.5px] p-2 md:p-3 text-right font-black text-sm md:text-base border-l-2 bg-gray-50 print:bg-transparent">अखेर शिल्लक<br/>(Closing Balance)</th>
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    {reportData.length === 0 && (
                      <tr>
                        <td colSpan={7} className="border border-black p-8 text-center text-sm font-bold text-gray-400">माहिती उपलब्ध नाही (No Data Available)</td>
                      </tr>
                    )}
                  </tbody>
                </table>
              ...
              <div className="flex justify-between items-end px-8 mt-16 pt-8 pb-4">
                <div className="text-center">
                  <div className="border-b border-black w-48 mb-2"></div>
                  <div className="font-bold text-xs md:text-sm">शालेय पोषण आहार प्रभारी</div>
                </div>
                {isSaved && (
                  <div className="text-center font-black text-indigo-700/20 text-4xl transform -rotate-12 select-none print:text-black/10">
                    APPROVED ARCHIVE
                  </div>
                )}
                <div className="text-center">
                  <div className="border-b border-black w-48 mb-2"></div>
                  <div className="font-bold text-xs md:text-sm">मुख्याध्यापक (स्वाक्षरी व शिक्का)</div>
                </div>
              </div>
