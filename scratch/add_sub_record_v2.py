...
def add_subscription_record():
    ...
    with engine.connect() as conn:
        ...
        insert_query = text("""
            INSERT INTO saas_subscriptions (id, teacher_id, plan_type, amount_paid, payment_status, created_at)
            VALUES (:id, :teacher_id, 'COMBO', 1199, 'PAID', :created_at)
        """)
        ...
        conn.commit()
        print(f"Added subscription history record for {email} with ID {new_id}")
